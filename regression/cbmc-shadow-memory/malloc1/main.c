...
void main()
{
  __CPROVER_field_decl_global("dr_write", (unsigned __CPROVER_bitvector[2]) 0);
  ...
  __CPROVER_set_field(&local_main_Lkeys, "dr_write", 2u);

  assert(__CPROVER_get_field(local_Lkeys_thread_ver, "dr_write") == 0);
  assert(__CPROVER_get_field(local_Lkeys_thread_ver + 1, "dr_write") == 1);
  assert(__CPROVER_get_field(&local_main_Lkeys, "dr_write") == 2u);
}
