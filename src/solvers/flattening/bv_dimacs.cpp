...
void bv_dimacst::show_prop_vars()
{
  ...
  for(const auto &s : get_symbols())
  {
    ...
    if(s.second.is_constant())
    {
      ...
      {
        log.debug() << s.first << " " << (s.second.is_true() ? "TRUE" : "FALSE")
                    << messaget::eom;
      }
    }
    else
    {
      ...
      {
        log.debug() << s.first << " " << s.second.dimacs() << messaget::eom;
      }
    }
  }
    ...
    if(has_suffix(id2string(m.first), "#1"))
    {
      ...
      for(const auto &lit : literal_map)
      {
        if(!lit.is_set)
          ...
          log.result() << " " << (lit.l.is_true() ? "TRUE" : "FALSE");
        else
          log.result() << " " << lit.l.dimacs();
      }
    ...
    else
    {
      ...
      for(const auto &lit : literal_map)
      {
        if(!lit.is_set)
          ...
          log.debug() << " " << (lit.l.is_true() ? "TRUE" : "FALSE");
        else
          log.debug() << " " << lit.l.dimacs();
      }
