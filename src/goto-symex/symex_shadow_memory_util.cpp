  ...
  log.conditional_output(
    ...
    [ns, shadowed_address, resolved_expr](messaget::mstreamt &mstream) {
      mstream << "Shadow memory: exact match: "
              << from_expr(shadowed_address.address)
              << " == " << from_expr(resolved_expr) << messaget::eom;
    });
#endif
}
  ...
  const value_set_dereferencet::valuet &shadow_dereference)
{
  ...
  log.conditional_output(
    ...
    [ns, shadowed_address, expr, dereference, matched_base_address, shadow_dereference](
      ...
      mstream << "Shadow memory:   base: " << from_expr(shadowed_address.address)
              << " <-- " << from_expr(matched_base_address) << messaget::eom;
      mstream << "Shadow memory:   cell: " << from_expr(dereference.pointer) << " <-- "
              << from_expr(expr) << messaget::eom;
      mstream << "Shadow memory:   shadow ptr: "
              << from_expr(shadow_dereference.pointer) << messaget::eom;
      mstream << "Shadow memory:   shadow val: "
              << from_expr(shadow_dereference.value) << messaget::eom;
    });
#endif
}
  ...
  const exprt &expr)
{
  ...
  log.conditional_output(
    log.debug(), [ns, address, expr](messaget::mstreamt &mstream) {
      mstream << "Shadow memory: value set match: " << from_expr(address)
              << " <-- " << from_expr(expr) << messaget::eom;
    });
#endif
}
  ...
  const exprt &cond)
{
  ...
  log.conditional_output(
    log.debug(), [ns, cond_text, cond](messaget::mstreamt &mstream) {
      mstream << "Shadow memory: " << cond_text << ": " << from_expr(cond)
              << messaget::eom;
    });
#endif
}
  ...
  const std::vector<exprt> &value_set)
{
  ...
  log.debug(), [ns, value_set](messaget::mstreamt &mstream) {
    ...
    {
      mstream << "Shadow memory: value set: " << from_expr(e) << messaget::eom;
    }
  });
#endif
}
  ...
  log.conditional_output(
    log.debug(), [field_name, ns, expr, value](messaget::mstreamt &mstream) {
      mstream << "Shadow memory: set_field: " << id2string(field_name) << " for "
              << from_expr(expr) << " to " << from_expr(value)
              << messaget::eom;
    });
}
