  ...
  const exprt &expr)
{
  ...
  if(type.id() == ID_array)
  {
    ...
    {
      if(!can_be_initialized_rec(
           ns, index_exprt(expr, from_integer(index, signed_long_int_type()))))
      {
        return false;
      }
    }
  ...
  std::map<irep_idt, typet> &fields)
{
  ...
  else if(type.id() == ID_struct && !per_object)
  {
    ...
    {
      initialize_rec(
        ns, state, member_exprt(expr, component), per_object, fields);
    }
    return;
  }
      ...
      log.conditional_output(
        ...
        [field, ns, address_expr, per_object](messaget::mstreamt &mstream) {
          mstream << "initialize field " << id2string(field.get_identifier())
                  ...
                  << (per_object ? " (per object)" : "") << messaget::eom;
        });
    }
  }
}
  ...
  const typet &field_type)
{
  ...
  addresses.push_back(goto_symex_statet::shadowed_addresst{
    expr, new_symbol.symbol_expr(), per_object});
...
static optionalt<exprt> set_field_per_object(
  const namespacet &ns,
  const messaget &log,
  const value_setst::valuest &value_set,
...
static optionalt<exprt> get_field_per_object(
  const namespacet &ns,
  const messaget &log,
  const value_setst::valuest &value_set,
  const goto_symex_statet::shadowed_addresst &shadowed_address,
...
/// Return a new lhs if an element was added to the lhs
static optionalt<exprt> set_field_per_element(
  const namespacet &ns,
  const messaget &log,
  const value_setst::valuest &value_set,
  const goto_symex_statet::shadowed_addresst &shadowed_address,
  ...
  const exprt &lhs)
{
  ...
  if(!cond.is_false())
  {
    ...
    return if_exprt(cond, address_of_exprt(shadowed_address.shadow), lhs);
  }
  return {};
}

/// Return a new rhs if an element was added to the rhs
static optionalt<exprt> get_field_per_element(
  const namespacet &ns,
  const messaget &log,
  const value_setst::valuest &value_set,
  const goto_symex_statet::shadowed_addresst &shadowed_address,
  ...
  const code_function_callt &code_function_call)
{
  ...
  for(const auto &shadowed_address : addresses)
  {
    ...
    {
      maybe_lhs = set_field_per_object(
    ...
    for(mp_integer index = 0; index < size; ++index)
    {
      initialize_rec(
        ...
        dereference_exprt(plus_exprt(
          address_of_exprt(expr), from_integer(index, signed_long_int_type()))),
        per_object,
        state.global_fields);
    }
  }
